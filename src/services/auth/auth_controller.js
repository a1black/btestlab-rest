...
function dataAccessor(req) {
  return userDataAccessor(req.context.db)
}
...
async function loginPasswordAuth(req, res) {
  const { error, value: input } = authSchema
    .loginPassword(req.config('input.auth'))
    .validate(req.body ?? {})

  const user = error ? null : await dataAccessor(req).read(input.login)
  const matched = user?.password
    ? await verifyPassword(input.password, user.password)
    : false

  if (user && !user.password) {
    req.logger.error('auth:empty_password:id:' + user._id)
  }
